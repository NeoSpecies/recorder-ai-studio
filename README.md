...
- 输出目录化：每次单音频转写都会生成独立输出目录，并复制一份源音频到该目录，便于归档、移动和复核。
- MCP 任务接续：异步 job 元数据持久化到本地；MCP 重启后可扫描 queued/running 未完成任务并恢复跟进，已完成任务可继续查询结果。
...
输出会写入独立运行目录，例如 `../outputs/agent-runs/meeting-demo-20260703-153000/`。目录内包括：

- 源音频副本：保留原文件名，便于和结果一起归档；不会删除或移动原始音频。
...
可暴露十三个工具，分为模型运行、异步转写、智能体校准、词库交互四组：

| 工具 | 说明 |
| --- | --- |
| `recorder_asr_status` | 查询本地 FunASR / SenseVoiceSmall 模型和运行时保活状态 |
| `recorder_resume_jobs` | 扫描持久化 job，恢复 MCP 重启或连接中断前未完成的 queued/running 任务 |
...
- 源音频副本：复制到当前 job 的 `artifacts/`，不会删除或移动原始音频。
...
任务接续边界：job 元数据会持久化到 `rec-xxxx.json`；MCP 启动时会扫描 `queued` / `running` 任务，把上一进程断开的 running 任务重新置为 `queued` 并提交执行；如果发现 report 产物已写完但 job 未更新，会从产物恢复为 `completed`。当前版本恢复粒度是 job 级与已完成产物级；FunASR 内部分片还没有逐 chunk checkpoint，因此中途失败的长音频会从该 job 重新执行 ASR，但不会丢失 jobId、日志、错误和已完成结果。
...
| `RECORDER_AI_MCP_JOB_DIR` | `../outputs/mcp-jobs` | MCP job、日志和每个 job 的 `artifacts/` 结果输出目录。 |
...
### `recorder_resume_jobs`

用途：扫描本地持久化 job 文件，恢复 MCP 重启、断网、WorkBuddy 连接器重启前未完成的任务。

恢复规则：

- `queued` / `running`：重新置为 `queued` 并提交后台 worker 继续执行。
- 已经写完 report 但 job 状态没更新：从产物恢复为 `completed`。
- `completed`：保持完成状态，可继续通过 `recorder_job_status` / `recorder_job_result` 查询。
- 当前恢复粒度为 job 级；如果 FunASR 在长音频中途失败，会保留同一个 `jobId`、日志和输出目录，并重新执行该 job。

自然语言示例：

```text
请扫描 recorder-ai-studio 里上次未完成的转写任务，并恢复可继续执行的 queued/running job。恢复后告诉我哪些任务被恢复、哪些已经从产物恢复为 completed。
```
...
5. 断点续跑：当前已支持 MCP job 级恢复和已完成产物恢复；后续可继续增强为已完成分片落盘缓存，失败后只重跑未完成片段。
